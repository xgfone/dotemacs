...
<b>Features——本配置的特性</b>
    ...
    4、额外实现的小功能及其快捷键绑定：
      ...
      （20）增加Emacs内建的正则表达式字符串高亮
           ...
           注：由于 Emacs 24 以下版本不支持 defvar-local，而本配置又要兼容版本 23，因此，本配置定义了一个 
           my-defvar-local 符号（为了与 defvar-local 区别）————可以定义一个局部于缓冲区的变量。但 read-regexp
           24以前的 Emacs 版本无法直接使用它，因此，在24版本以前，各个缓冲区间并不是 100% 的分离，即在输入搜索
           字符串正则表达式时，会有一个默认的提示（上一次搜索的字符串正则表达式，如果输入为空，则使用上一次的
           搜索规则），而这个提示是所有缓冲区共用的。在 Emacs 24（以其以后的版本），read-regexp 可以直接指定局
           部变量，可以做到 100% 分离。
